...
export default function App() {
  ...
  const isAuthenticated = authState[0];
  const setIsAuthenticated = authState[1];

  const coursesState = useState([
    { id: 1, title: 'Advanced Algorithms (CS401)', enrolled: 42, status: 'Active' },
    { id: 2, title: 'UI/UX Architecture (DS205)', enrolled: 85, status: 'Active' }
  ]);
  const courses = coursesState[0];
  const setCourses = coursesState[1];

  const globalCourseContextState = useState('');
  const activeCourseId = globalCourseContextState[0];
  const setActiveCourseId = globalCourseContextState[1];
    ...
    <Router>
      <div style={{ display: 'flex' }}>
        ...
        <div className="page-container">
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '24px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                ...
                <select className="input-field" value={activeCourseId} onChange={function(e) { setActiveCourseId(e.target.value) }} style={{ margin: 0, padding: '4px 8px', width: '200px' }}>
                  ...
                  {courses.map(function(c) { return <option key={c.id} value={c.id}>{c.title}</option> })}
                </select>
              </div>
              <button
          ...
          <Routes>
            <Route path="/" element={<CourseManagement courses={courses} setCourses={setCourses} />} />
