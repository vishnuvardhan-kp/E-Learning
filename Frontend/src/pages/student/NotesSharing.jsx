...
export default function NotesSharing() {
  ...
  const fetchNotes = async () => {
    try {
      const res = await fetch(__API_URL__ + '/notes');
  ...
  const handleSubmit = async (e) => {
    ...
    if (newTitle && file) {
        try {
            // Simulate file URL (in real apps, upload to cloud storage)
            const fileUrl = URL.createObjectURL(file);
            
            await fetch(__API_URL__ + '/notes', {
                method: 'POST',
  ...
  const handleUpvote = async (id) => {
    try {
        await fetch(`__API_URL__/notes/${id}/upvote`, {
            method: 'PUT'
        });
        fetchNotes();
    } catch(e) {
        ...
        {notes.map(note => (
            <div key={note._id} className="card">
              ...
              <div className="card-actions" style={{ display: 'flex', gap: '8px', marginTop: '16px' }}>
                ...
                <button className="action-button" style={{ background: '#e2e8f0', color: '#0f172a', flex: 1, padding: '10px 12px', fontSize: '11px' }} onClick={() => handleUpvote(note._id)}>
                    Upvote ({note.upvotes})
                </button>
              </div>
            </div>
