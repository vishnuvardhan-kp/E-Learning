export default function Progress({ enrolledCourses = [] }) {
  // Simple deterministic progress calculator: pre-existing courses have high progress, newly enrolled have 0%
  const getCourseProgress = (id) => (id > 100 ? Math.floor(Math.random() * 20) + 65 : 0);
  
  // Calculate total course completion % dummy logic
  const totalCourses = enrolledCourses.length;
  const avgCompletion = totalCourses > 0 
    ? Math.floor(enrolledCourses.reduce((sum, course) => sum + getCourseProgress(course.id), 0) / totalCourses)
    : 0;
